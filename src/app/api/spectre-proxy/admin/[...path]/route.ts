...
const ENV_FILE = path.join(HOME, ".spectre-proxy", ".env");
const ENV_DIR = path.join(HOME, ".spectre-proxy");
...
async function migrateLegacyEnv(): Promise<void> {
  ...
  try {
    ...
    console.log("[spectre] migrated config from ~/.fcc/.env → ~/.spectre-proxy/.env");
  } catch { /* best-effort */ }
}
