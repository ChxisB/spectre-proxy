...
// event into a closure nobody reads, and subscribers (`spectre run`)
...
// publish a RunComplete for its RunID, hanging a `spectre run` caller that
...
// than internal bookkeeping, which is the contract a `spectre run` caller
...
// bookkeeping slice, not the published event a `spectre run` caller awaits.
