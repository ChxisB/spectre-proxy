...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "spectre.log")
...
func TestNewSpectreLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewSpectreLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, SpectreLogsToolName, tool.Info().Name)
}

func TestSpectreLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 3})
...
func TestSpectreLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 0})
...
func TestSpectreLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestSpectreLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runSpectreLogs("/nonexistent/path/spectre.log", SpectreLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestSpectreLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "spectre.log")
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestSpectreLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "spectre.log")
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 10})
...
func TestSpectreLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 1})
...
func TestSpectreLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestSpectreLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestSpectreLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestSpectreLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "spectre.log")
...
func TestSpectreLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "spectre.log")
...
func TestSpectreLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestSpectreLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestSpectreLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestSpectreLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runSpectreLogs(logFile, SpectreLogsParams{Lines: 5})
...
func TestSpectreLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
