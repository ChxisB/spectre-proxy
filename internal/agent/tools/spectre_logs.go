...
const SpectreLogsToolName = "spectre_logs"

//go:embed spectre_logs.md.tpl
var spectreLogsDescriptionTmpl []byte

var spectreLogsDescriptionTpl = template.Must(
	template.New("spectreLogsDescription").
		Parse(string(spectreLogsDescriptionTmpl)),
)

type spectreLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func spectreLogsDescription() string {
	return renderTemplate(spectreLogsDescriptionTpl, spectreLogsDescriptionData{
...
type SpectreLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewSpectreLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		SpectreLogsToolName,
		spectreLogsDescription(),
		func(ctx context.Context, params SpectreLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runSpectreLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runSpectreLogs reads and formats the last N log entries from the given file.
func runSpectreLogs(logFile string, params SpectreLogsParams) string {
