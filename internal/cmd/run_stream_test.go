...
// with reason=tool_use and used to terminate `spectre run` early via
...
// subscribers (e.g. a TUI watching workspace events while spectre
...
// non-nil error from `spectre run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `spectre run` waits) exits cleanly rather
...
// queue scenario: `spectre run --continue` attaches to a session
