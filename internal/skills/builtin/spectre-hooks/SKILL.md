---
name: spectre-hooks
description: Use when the user wants to add, write, debug, or configure a Spectre hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in spectre.json.
---

# Spectre Hooks

Hooks are user-defined commands in `spectre.json` that fire at specific points
...
| `SPECTRE_TOOL_NAME`            | Tool being called (e.g. `bash`)          |
| `SPECTRE_SESSION_ID`           | Current session ID                       |
| `SPECTRE_CWD`                  | Working directory                        |
| `SPECTRE_PROJECT_DIR`          | Project root directory                   |
| `SPECTRE_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command      |
| `SPECTRE_TOOL_INPUT_FILE_PATH` | For file tools: the target file path     |
...
if echo "$SPECTRE_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
if [[ "$SPECTRE_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
3. Add the entry under `hooks.PreToolUse` in `spectre.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Spectre logs.
...
Spectre also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Spectre treats `updated_input` as shallow-merge, Claude Code replaces.
