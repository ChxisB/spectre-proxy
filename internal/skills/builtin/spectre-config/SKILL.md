---
name: spectre-config
description: Use when the user needs help configuring Spectre — working with spectre.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Spectre behavior.
---

# Spectre Configuration

Spectre uses JSON configuration files with the following priority (highest to lowest):

1. `.spectre.json` (project-local, hidden)
2. `spectre.json` (project-local)
3. `$XDG_CONFIG_HOME/spectre/spectre.json` or `$HOME/.config/spectre/spectre.json` (global)

## Basic Structure

```json
{
  "$schema": "https://github.com/ChxisB/spectre-proxy/blob/main/spectre.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
Spectre runs selected string fields through an embedded bash-compatible
...
`spectre.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Spectre in a directory whose `spectre.json` you haven't
reviewed.
...
```json
{
  "options": {
    ...
    "disabled_skills": ["spectre-config"],
    "tui": {
      "compact_mode": false,
      "diff_mode": "unified",
      "transparent": false
...
> `.agents/skills`, `.spectre/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".spectre/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".spectre/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `SPECTRE_EVENT` | Event name (e.g. `PreToolUse`) |
| `SPECTRE_TOOL_NAME` | Name of the tool being called |
| `SPECTRE_SESSION_ID` | Current session ID |
| `SPECTRE_CWD` | Current working directory |
| `SPECTRE_PROJECT_DIR` | Project root directory |
| `SPECTRE_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `SPECTRE_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Spectre also supports the Claude Code hook output format:
...
- `SPECTRE_GLOBAL_CONFIG` - Override global config location
- `SPECTRE_GLOBAL_DATA` - Override data directory location
- `SPECTRE_SKILLS_DIR` - Override default skills directory
