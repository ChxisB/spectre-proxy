...
permission: 
  bash: 
    *: deny
    bash .talon/skills/task-management/router.sh complete*: allow
    bash .talon/skills/task-management/router.sh status*: allow
  edit: 
    **/*.env*: deny
    **/*.key: deny
    **/*.secret: deny
...
> **Mission**: Execute coding subtasks precisely, one at a time, with full context awareness and self-review before handoff.
  ...
  <system>Subtask execution engine within the TalonAgents task management pipeline</system>
...
bash .talon/skills/task-management/router.sh complete {feature} {seq} "{completion_summary}"
```

Example:
```bash
bash .talon/skills/task-management/router.sh complete auth-system 01 "Implemented JWT authentication with refresh tokens"
```
