...
permission: 
  bash: 
    *: deny
    ...
    bash .talon/skills/task-management/router.sh*: allow
  edit: 
    **/*.env*: deny
    **/*.key: deny
    **/*.secret: deny
...
<system>Parallel execution coordinator within the TalonAgents task management pipeline</system>
...
The orchestrator (TalonCoder/TalonAgent) provides:
...
3. **Manually mark complete** if needed:
   ```bash
   bash .talon/skills/task-management/router.sh complete {feature} {seq} "{summary}"
   ```
