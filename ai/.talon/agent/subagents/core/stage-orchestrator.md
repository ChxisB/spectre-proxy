...
permission: 
  bash: 
    *: deny
    ...
    bash .talon/skill/task-management/router.sh*: allow
  edit: 
    **/*.env*: deny
    **/*.key: deny
    **/*.secret: deny
...
<system>Multi-stage workflow coordinator within the TalonAgents orchestration pipeline</system>
...
2. Initialize stage tracking:
   ```bash
   npx ts-node .talon/skill/task-management/scripts/stage-cli.ts init {feature}
   ```
...
1. **Validate Prerequisites**
   ```bash
   npx ts-node .talon/skill/task-management/scripts/stage-cli.ts validate {feature} {stage}
   ```
...
3. **Validate Stage Completion**
   ```bash
   npx ts-node .talon/skill/task-management/scripts/stage-cli.ts complete {feature} {stage}
   ```
...
3. **Execute Rollback (if needed)**
   ```bash
   npx ts-node .talon/skill/task-management/scripts/stage-cli.ts rollback {feature} {stage}
   ```
...
1. **Check Stage Status**
   ```bash
   npx ts-node .talon/skill/task-management/scripts/stage-cli.ts status {feature}
   ```
...
2. **Execute Abort**
   ```bash
   npx ts-node .talon/skill/task-management/scripts/stage-cli.ts abort {feature}
   ```
