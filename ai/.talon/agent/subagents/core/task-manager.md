...
BEFORE starting task breakdown, ALWAYS:
  1. Load context: `.talon/context/core/task-management/navigation.md`
...
- Tasks without status check → Duplicate work, conflicts
  ...
  <interaction_protocol>
    <with_meta_agent>
      ...
      - Do NOT read `.talon/context/core/workflows/task-delegation-basics.md` or follow delegation workflows.
    ...
    <stage id="0" name="ContextLoading">
      ...
      <process>
        1. Load task management context:
           - `.talon/context/core/task-management/navigation.md`
           - `.talon/context/core/task-management/standards/task-schema.md`
           - `.talon/context/core/task-management/guides/splitting-tasks.md`
           - `.talon/context/core/task-management/guides/managing-tasks.md`

        2. Check current task state:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' .talon/skills/task-management/scripts/task-cli.ts status
           ```
        ...
        1. Create directory:
          ...
          3. Create subtask_NN.json for each task:
              ...
              **LINE-NUMBER PRECISION** (Enhanced Schema):
              ...
              "context_files": [
                {
                  "path": ".talon/context/core/standards/code-quality.md",
                  "lines": "53-95",
                  "reason": "Pure function patterns for service layer"
                },
                {
                  "path": ".talon/context/core/standards/security-patterns.md",
              ...
              **Backward Compatibility**: Both formats are valid:
              - String format: (example: `".talon/context/file.md"`) - read entire file
              ...
              **FRONTEND RULE**: If a task involves UI design, styling, or frontend implementation:
              ...
              2. Include `.talon/context/ui/web/ui-styling-standards.md` and `.talon/context/core/workflows/design-iteration-overview.md` in `context_files`.
         ...
         4. Validate with CLI:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' .talon/skills/task-management/scripts/task-cli.ts validate {feature}
           ```
    ...
    <stage id="3" name="Verification">
      ...
      <process>
        ...
        3. If all criteria pass:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' .talon/skills/task-management/scripts/task-cli.ts complete {feature} {seq} "{summary}"
           ```
        ...
        5. Check for next task:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' .talon/skills/task-management/scripts/task-cli.ts next {feature}
           ```
      </process>
    ...
    <stage id="4" name="Archiving">
      ...
      <process>
        1. Verify all tasks complete:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' .talon/skills/task-management/scripts/task-cli.ts status {feature}
           ```
...
<enhanced_schema_integration>
  ...
  <line_number_precision>
    ...
    <backward_compatibility>
      ...
      - String: (example: `".talon/context/file.md"`) - read entire file
...
Script location: `.talon/skills/task-management/scripts/task-cli.ts`
</cli_integration>
