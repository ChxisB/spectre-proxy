...
- Default context root: .talon/context/
...
## 📋 Agent Pipeline — Stage-by-Stage Delegation

Follow this pipeline for complex features. See `workflows/agent-pipeline.md` for full details.

```
STAGE 1: DISCOVER  → ContextScout, ExternalScout
STAGE 2: PLAN      → ArchitectureAnalyzer, StoryMapper, PrioritizationEngine, TaskManager
STAGE 3: DEFINE    → ContractManager
STAGE 4: BUILD     → BatchExecutor → CoderAgent(s), OpenFrontendSpecialist, TestEngineer
STAGE 5: VERIFY    → CodeReviewer, BuildAgent, DocWriter
```
...
Pass work between agents using `task()`:

```javascript
// STAGE 1: DISCOVER — find context
task(subagent_type="ContextScout", description="Find auth standards", prompt="...")

// STAGE 2: PLAN — decompose the feature
task(subagent_type="ArchitectureAnalyzer", description="Analyze auth architecture", prompt="...")
task(subagent_type="TaskManager", description="Break auth into subtasks", prompt="...")

// STAGE 3: DEFINE — set contracts
task(subagent_type="ContractManager", description="Define auth API contracts", prompt="...")

// STAGE 4: BUILD — implement in parallel
task(subagent_type="BatchExecutor", description="Execute Batch 1 (subtasks 01-03)", prompt="...")
//   ^ BatchExecutor internally delegates to CoderAgent × N
          ...
          <decision id="execution_strategy">
            <condition test="batch_size_and_complexity">
              IF batch has 1-4 parallel tasks AND simple error handling:
                → Use DIRECT execution (TalonCoder → CoderAgents)
              IF batch has 5+ parallel tasks OR complex error handling needed:
                → Use BATCH EXECUTOR (TalonCoder → BatchExecutor → CoderAgents)
            </condition>
          </decision>
...
<execution_philosophy>
  ...
  **BatchExecutor Usage**: 
    - 1-4 parallel tasks: TalonCoder delegates directly to CoderAgents (simpler, faster setup)
    - 5+ parallel tasks: TalonCoder delegates to BatchExecutor (better monitoring, error handling)
  ...
  **Key Principle**: ContextScout discovers paths. TalonCoder persists them into context.md. TaskManager creates parallel-aware task structure. BatchExecutor manages simultaneous CoderAgent delegations. No re-discovery.
</execution_philosophy>
