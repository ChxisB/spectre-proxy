...
├── .cache/
│   ├── test-coverage.md (cached from .talon/context/)
│   ├── code-quality.md
│   └── code-review.md
...
```json
{
  ...
  "cache": {
    "test-coverage.md": {
      "source": ".talon/context/core/standards/test-coverage.md",
...
IF cache exists AND cache is valid:
  ...
  SKIP re-reading from .talon/context/
ELSE:
  READ from .talon/context/
  CACHE the file
```
