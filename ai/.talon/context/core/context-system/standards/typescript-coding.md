...
This document defines TypeScript coding standards specifically for the context system. These standards are extracted from the main Talon TypeScript standards and tailored for context-related code.
...
- **Talon TypeScript** (`talonagents-repo/standards/talon-typescript.md`) - Talon-specific patterns (fn() wrapper, namespaces, etc.)
...
- Writing Talon-specific code → Load talon-typescript.md
- Writing context system code specifically → Load universal typescript.md, talon-typescript.md, AND this file
...
describe('ContextResolver', () => {
  ...
  describe('resolve', () => {
    ...
    it('should resolve root context reference', async () => {
      ...
      expect(result).toBe('.talon/context/core/standards/code-quality.md');
    });
...
import { ContextResolver } from '@talonagents/core';

const program = new Command();

program
  .name('oac')
  .description('TalonAgents Context CLI')
  .version('1.0.0');
...
export const OACConfigSchema = z.object({
  ...
  context: z.object({
    root: z.string().default('.talon/context'),
...
- Talon TypeScript standards: `talonagents-repo/standards/talon-typescript.md`
- Context system guide: `.talon/context/core/context-system/guides/creation.md`
- Context structure: `.talon/context/core/context-system/standards/structure.md`
