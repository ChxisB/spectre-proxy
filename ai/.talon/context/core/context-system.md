...
### 0. RTK (Reference Text Knowledge) ⭐ NEW
Replace inline verbose content with compact `@ref:` codes. Agents load full content on demand.
**Goal**: Reduce tokens by 80%+. See `guides/rtk.md` for implementation.
**Combined with MVI**: MVI keeps files <200 lines; RTK keeps prompts referencing those files instead of embedding them.
...
**Use when**: Content is repository-specific (e.g., `talonagents-repo/`)
...
**Use for**: Repository-specific context (e.g., `talonagents-repo/`)
...
Location: `.talon/context/core/standards/`
