...
- **essential** - Minimal setup (talonagent + core subagents)
...
// ✅ Agent exists in components
{
  ...
  "path": ".talon/agent/subagents/development/devops-specialist.md"
}
...
5. **Test actual install**:
   ...
   cd /tmp/test-install
   REGISTRY_URL="file://$(pwd)/registry.json" bash <(curl -s https://raw.githubusercontent.com/darrenhinde/TalonAgentsControl/main/install.sh) developer
   
   # Check if agent was installed
   ls .talon/agent/category/your-agent.md
   ```
...
*Note: Development category includes agents (taloncoder) and specialist subagents (frontend, devops)
