...
```typescript
'contextscout': 'talonagent',  // Maps subagent → parent
```
...
```bash
# Check agent file
cat .talon/agent/subagents/core/contextscout.md | head -20

# Verify frontmatter
grep -A 5 "^id:" .talon/agent/subagents/core/contextscout.md
```
...
```json
{
  ...
  "model": "talon/grok-code-fast",
  "timestamp": "2026-01-07T..."
}
```

**Red Flags**:
- `"agent": "core/talonagent"` ← Wrong! TalonAgent is running instead
...
Tool Call Details:
  1. glob: {"pattern":"*.md","path":".talon/context/core"}
```
...
prompts:
  - text: |
      List all markdown files in .talon/context/core/
```
...
prompts:
  - text: |
      Use the glob tool to find all markdown files in .talon/context/core/
      
      You MUST use the glob tool like this:
      glob(pattern="*.md", path=".talon/context/core")
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| TalonAgent runs instead | Used `--agent` flag | Use `--subagent` flag |
