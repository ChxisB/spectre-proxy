...
**Purpose**: Complete example of creating an Talon skill
...
```bash
mkdir -p .talon/skills/task-management/scripts
```

```
.talon/skills/task-management/
├── SKILL.md
├── router.sh
└── scripts/
    └── task-cli.ts
...
npx ts-node .talon/skills/task-management/scripts/task-cli.ts status

# Show next eligible tasks
npx ts-node .talon/skills/task-management/scripts/task-cli.ts next

# Mark complete
npx ts-node .talon/skills/task-management/scripts/task-cli.ts complete <feature> <seq> "summary"
```
```
