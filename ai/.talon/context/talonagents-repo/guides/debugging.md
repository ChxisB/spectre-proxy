...
# Test core agents
cd evals/framework && npm run eval:sdk -- --agent=core/talonagent --pattern="smoke-test.yaml"
```
...
```bash
# Check frontmatter
head -10 .talon/agent/{category}/{agent}.md
...
```bash
# Add frontmatter
cat > .talon/agent/{category}/{agent}.md << 'EOF'
---
...
```bash
# Check file exists
ls -la .talon/agent/subagents/development/frontend-specialist.md
...
./scripts/registry/auto-detect-components.sh --path .talon/agent/development/
```
...
./scripts/registry/validate-registry.sh -v
./scripts/validation/validate-test-suites.sh
cd evals/framework && npm run eval:sdk -- --agent=core/talonagent
```
