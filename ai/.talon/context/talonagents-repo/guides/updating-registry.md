...
Would update:
  - agent: core/talonagent (description changed)
```
...
**Command** (`.talon/command/your-command.md`):
```yaml
---
...
**Subagent** (`.talon/agent/subagents/category/your-agent.md`):
```yaml
---
id: your-agent
...
**Context** (`.talon/context/category/your-context.md`):
```yaml
---
...
- `agent:` - References a main agent (e.g., `agent:talonagent`)
...
```json
{
  ...
  "name": "Agent Name",
  "type": "agent",
  "path": ".talon/agent/category/agent-name.md",
...
```bash
# 1. Create .talon/command/my-command.md with frontmatter:
cat > .talon/command/my-command.md << 'EOF'
---
...
```bash
# 1. Edit .talon/command/existing-command.md frontmatter:
# Add or update:
...
```json
{
  ...
  "type": "context",
  "path": ".talon/context/core/workflows/session-management.md",
  "aliases": [
    "workflows-sessions",
    "sessions"
  ],
