...
# Guide: Adding an Talon Skill (Basics)

**Prerequisites**: Load `plugins/context/capabilities/events_skills.md` first  
**Purpose**: Create an Talon skill directory and SKILL.md file

**Note**: This is for **Talon skills** (internal system). For **Claude Code Skills**, see `creating-skills.md`.

---

## Overview

Adding an Talon skill involves:
...
```bash
mkdir -p .talon/skills/{skill-name}/scripts
```

**Standard structure**:
```
.talon/skills/{skill-name}/
...
```bash
npx ts-node .talon/skills/{skill-name}/scripts/skill-cli.ts command1
```
...
```bash
chmod +x .talon/skills/{skill-name}/router.sh
```
