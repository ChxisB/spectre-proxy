...
# Guide: Talon Skill Implementation

**Prerequisites**: Load `adding-skill-basics.md` first  
**Purpose**: CLI implementation, registry, and testing for Talon skills
  ...
  "skills": [
    {
      ...
      "name": "Skill Name",
      "type": "skill",
      "path": ".talon/skills/{skill-name}/SKILL.md",
...
```bash
# Test help
bash .talon/skills/{skill-name}/router.sh help

# Test commands
bash .talon/skills/{skill-name}/router.sh command1 --option value

# Test with npx
npx ts-node .talon/skills/{skill-name}/scripts/skill-cli.ts help
```

### Test Talon Integration

1. Call skill via Talon
...
- [ ] `.talon/skills/{skill-name}/SKILL.md` created
- [ ] `.talon/skills/{skill-name}/router.sh` created (if CLI-based)
