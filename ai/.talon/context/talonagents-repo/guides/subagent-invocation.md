<!-- Context: talonagents-repo/guides | Priority: high | Version: 1.0 | Updated: 2026-02-15 -->
...
**Root Cause**: The `subagent_type` parameter in the task tool must match the registered agent type in the Talon CLI, not the file path.
...
Based on the Talon CLI registration, use these exact strings for `subagent_type`:
...
// ❌ Using registry path
task(
  subagent_type=".talon/agent/subagents/core/task-manager.md",
  ...
)
```
...
**Status**: ⚠️ May not be registered in Talon CLI yet

The `ContextScout` subagent exists in the repository but may not be registered in the Talon CLI's available agent types.
...
glob(pattern="**/*.md", path=".talon/context")

// 2. Use grep to search content
grep(pattern="registry", path=".talon/context")

// 3. Read relevant files directly
read(filePath=".talon/context/talonagents-repo/core-concepts/registry.md")
```
...
2. **taloncoder.md** - Check if uses incorrect format

### Fix Process

1. **Find incorrect invocations**:
   ```bash
   grep -r 'subagent_type="subagents/' .talon/agent --include="*.md"
   ```
...
2. Verify subagent exists in `.talon/agent/subagents/`
...
- **Subagents**: `.talon/agent/subagents/` - Subagent definitions
