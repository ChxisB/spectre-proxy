<!-- Context: talonagents-repo/standards/agent-frontmatter | Priority: critical | Version: 1.0 | Updated: 2026-01-31 -->
# Standard: Agent YAML Frontmatter

**Purpose**: Valid Talon agent frontmatter structure and common mistakes to avoid  
...
Agent frontmatter must contain ONLY valid Talon fields. All other metadata (id, name, category, tags, dependencies) belongs in `.talon/config/agent-metadata.json`.

**Why**: Talon validates frontmatter strictly. Extra fields cause validation errors.
...
```
**Fix**: Move to `.talon/config/agent-metadata.json`:
```json
{
  "agents": {
    "my-agent": {
...
- **Talon Docs**: https://talon.ai/docs/agents/
