<!-- Context: talonagents-repo/standards/navigation | Priority: critical | Version: 1.0 | Updated: 2026-01-31 -->
# TalonAgents Repo Standards

**Purpose**: Standards for creating and maintaining agents in TalonAgents Control  
...
- Valid Talon YAML frontmatter structure
...
|------|-------------|----------|-------|
| [agent-frontmatter.md](agent-frontmatter.md) | Valid Talon frontmatter fields and common mistakes | critical | <200 |
...
### Valid Talon Fields (Frontmatter)
...
These fields are NOT valid in Talon frontmatter:
- `id` - Agent identifier
...
**Solution**: Move these to `.talon/config/agent-metadata.json`
...
### Check YAML Syntax
```bash
yq eval '.talon/agent/category/agent.md' > /dev/null
```

### Check for Duplicate Keys
```bash
grep -A 50 "^---$" .talon/agent/category/agent.md | grep -E "^[a-z_]+:" | sort | uniq -d
```

### List All Frontmatter Keys
```bash
grep -A 50 "^---$" .talon/agent/category/agent.md | grep -E "^[a-z_]+:" | cut -d: -f1
```
...
- Invalid Talon fields in frontmatter

---

**Version**: 1.0.0  
**Maintainer**: TalonAgents Control Team
