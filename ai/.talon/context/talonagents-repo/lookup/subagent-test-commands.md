...
```bash
npm run eval:sdk -- --agent=core/talonagent --pattern="delegation/*.yaml"
```

### Test Specific Delegation
```bash
npm run eval:sdk -- --agent=core/talonagent --pattern="delegation/01-contextscout-delegation.yaml"
```
...
head -30 .talon/agent/subagents/core/contextscout.md

# Check tool permissions
grep -A 10 "^tools:" .talon/agent/subagents/core/contextscout.md
```
...
| `--agent` | Test primary agent (or delegation) | `--agent=core/talonagent` |
...
### Agent Files
```
.talon/agent/subagents/core/{subagent}.md
```
