...
import type { useTalonKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useTalonKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getTalonModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getTalonModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
