...
Move the canonical Talon terminal application from
...
The SDK is the TUI's Talon boundary. Missing backend data or operations must
...
- Talon domain data displayed by the TUI
...
route, and feature-plugin code no longer reads Talon globals or process state;
...
Replace process-global Talon state with resolved TUI inputs.
...
adapters rather than Talon domain implementations.

Make the SDK the only Talon domain boundary used by the TUI.
...
- Domain-facing TUI code imports Talon data and operations only from
  `@talon-ai/sdk`.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Talon domain state.
